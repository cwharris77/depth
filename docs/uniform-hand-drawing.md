...
- Trace an outline element from the UNION of it and everything it encloses, with holes filled (`fill_holes`), never from its own ink alone — the body ink touching it breaks the outline into slivers, and the antialiased seam between two inks matches neither predicate.
- `trace(..., space=)` chooses what 0..100 means. `'image'` spreads the whole reference across the placement box; `'art'` uses the bounding box of everything traced, so a multi-component mark lands as one drawing instead of each component stretched to its own extents.
- Check the reference's own aspect against the placement box before fitting it. A flat logo file and the decal the same club puts on a shell are usually not the same drawing — the decal arches around the shell, so it is taller and its tails sweep further. Squeezing the flat one into the decal's box gets the extents right and the drawing wrong, because the stretch thickens every stroke with it. When the two aspects differ by more than a few percent, trace the mark off the reference that shows it ON the helmet, even when that reference is the coarser of the two (the Chargers' bolt: a 93px GUD helmet upsampled 8x beat a clean vector logo).
