...
**Every mark in this archive traces but one.** Of the 31 teams that carry a helmet mark — Cleveland wears no logo — 30 are authored. The single exception is Houston's navy-shell bull, and it takes *two* coincident problems to get there rather than the one that was claimed of six other marks before they were traced anyway. The history of the old does-not-trace column is the most useful thing in this section, so it is kept below rather than deleted.
...
| Not reachable | A mark whose body is the shell color AND whose keyline is dashed rather than closed | Nothing in this toolkit — see Houston below |
...
**When a mark shares its color with its shell, check whether another kit wears it on a different shell — but verify it is the same mark before you reuse it.** The idea is sound and the check is the part that matters. Houston is the cautionary case: its navy shells wear a compact bull head, and its Battle Red shell wears a large stylized horn sweeping across the crown. **These are two different marks**, and a pass that assumed they were one mark drawn with and without its detail shipped the horn onto the home and away helmets, where it is simply wrong.

What made that slip through was a weak registration check: the away figure's star and red half landed *inside* the Battle Red silhouette, which was read as confirmation. They will land inside almost any large shape. **Compare the outlines** — bbox, aspect ratio, and a visual overlay of the two contours — not whether the small features fall somewhere within.

**Houston's navy bull is the one mark in this archive that is genuinely unreachable, and it takes two coincident problems to get there.** Its body is exactly the shell navy `(3,24,37)`, so nothing selects it; *and* its white keyline is drawn **dashed** — dozens of fragments with 1–3px gaps — so nothing encloses it either. A morphological closing does not rescue it: bridging gaps that wide needs a dilation of ~2.8 reference px, and eroding back by the same amount guts the horns, which are thinner than that. Traced as-is the dashes render as scattered debris, and at swatch size the mark reads as a smudge — verified at actual size, not assumed. Either problem alone is survivable. Both together are not.

**A closed keyline is a separator; a broken one is not.** This is what distinguishes Atlanta from Houston, and it is the thing to check when a mark's body is the shell color. The falcon's body is the same black as its shell, which sounds fatal — but its white keyline is an unbroken ring, so filling that ring's hole yields the silhouette, and the body then falls out as the black component that does not touch the crop border (the shell's black does). Houston's keyline is the same idea dashed, which is why the identical collision is fatal there and harmless here. Before giving up on a body that collides with its shell, ask whether something *around* it closes — and before declaring it unreachable, confirm the keyline is broken rather than merely thin.
