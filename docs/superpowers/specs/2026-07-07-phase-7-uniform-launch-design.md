...
| Prod data | The schema **and** the curated-uniform seed apply as **migrations** via the Supabase GitHub integration on merge — no manual `ingest:uniforms` (that script was removed in PR-A; the curated seed is now a generated, append-only, source-guarded migration via `npm run gen:uniform-seed`). Confirm the migrations applied and spot-check two teams in prod. The ESPN ingest is already scheduled weekly and needs nothing. | Curated seed rides the normal migration pipeline, so prod picks it up on merge with no separate hand-run step. (Deploy-vs-migration race: on a schema merge, redeploy prod once the migration lands if team pages 404 — or land the migration in its own PR first.) |
