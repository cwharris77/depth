...
final class DepthUITests: XCTestCase {
    ...
    func testCompareRendersWebParityElements() throws {
        ...
        // DEP-277: chips built with `.frame(minHeight: 44)` can report back as
        // 43.99999999999997 through XCUITest's point↔pixel (3x) frame conversion — a
        // float-precision artifact, not an undersized chip. `>= 44` was failing on that
        // rounding noise once Compare picked up a real NavigationStack (one more layout
        // pass in the coordinate chain makes the rounding land under 44 more often); a
        // small epsilon tolerates the conversion noise without loosening the real 44pt
        // touch-target intent.
        XCTAssertTrue(
