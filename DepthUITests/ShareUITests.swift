...
final class ShareUITests: XCTestCase {
    ...
    func testShareDepthChartPresentsTheNativeShareSheet() throws {
        ...
        XCTAssertTrue(
            ...
            "the overflow menu should expose a Share entry point")
        shareButton.tap()

        let shareSheet = app.otherElements["ActivityListView"]
        XCTAssertTrue(
            shareSheet.waitForExistence(timeout: 10),
            "tapping Share should present the native share sheet")
        let brandedTitle = app.navigationBars.matching(
            NSPredicate(format: "identifier CONTAINS %@", "depth chart · The Sticks")
        ).firstMatch
        XCTAssertTrue(
            brandedTitle.waitForExistence(timeout: 5),
            "the share title should use the current brand")
        ...
        dismissRegion.tap()
        XCTAssertTrue(
            shareSheet.waitForAbsence(timeout: 10), "the share sheet should dismiss on cancel")
        XCTAssertTrue(
            overflow.waitForExistence(timeout: 5),
            "team detail should remain unchanged after cancelling the share sheet")
        overflow.tap()
        XCTAssertTrue(
            shareButton.waitForExistence(timeout: 5),
            "Share should still be reachable in the overflow menu")
    }
}
