...
#
# Constraint this satisfies: web/node_modules/ and web/.env.local are gitignored, so `git
# worktree add` produces a tree where `npm test`, `npm run format`, and the PreToolUse
# typecheck hook all fail on "command not found". This runs on SessionStart, is
# idempotent, and costs ~0ms once the worktree is warm.
#
...
#
# When web/package-lock.json differs from the main checkout's, the clone is stale for
...
#
# web/.env.local is symlinked, never copied — one copy of the secrets, on disk once.
set -uo pipefail
...
lock_hash() { shasum web/package-lock.json 2>/dev/null | cut -d' ' -f1; }

if [ ! -e web/node_modules ]; then
  if [ -d "$main_dir/web/node_modules" ]; then
    echo "worktree-bootstrap: cloning web/node_modules from $main_dir (APFS copy-on-write)" >&2
    cp -Rc "$main_dir/web/node_modules" web/node_modules 2>/dev/null || cp -R "$main_dir/web/node_modules" web/node_modules

    if ! cmp -s web/package-lock.json "$main_dir/web/package-lock.json"; then
      ...
      lock_was_clean=no
      git diff --quiet -- web/package-lock.json 2>/dev/null && lock_was_clean=yes
      lock_before=$(lock_hash)

      if ! (cd web && npm install --silent --no-audit --no-fund); then
        echo "worktree-bootstrap: npm install failed — falling back to npm ci" >&2
        (cd web && npm ci --silent) || echo "worktree-bootstrap: npm ci failed too; tree may be incomplete" >&2
      fi
      ...
      if [ "$lock_was_clean" = yes ] && [ "$(lock_hash)" != "$lock_before" ]; then
        git checkout -- web/package-lock.json 2>/dev/null &&
    ...
    echo "worktree-bootstrap: no web/node_modules in $main_dir — running npm ci" >&2
    (cd web && npm ci --silent)
  fi
fi

if [ ! -e web/.env.local ] && [ -f "$main_dir/web/.env.local" ]; then
  echo "worktree-bootstrap: linking web/.env.local from $main_dir" >&2
  ln -s "$main_dir/web/.env.local" web/.env.local
fi
