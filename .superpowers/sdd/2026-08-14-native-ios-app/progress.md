...
Task 9B: shipped `os_signpost` instrumentation (`ios/Depth/Support/DepthSignposts.swift`) around app-launch→first-render, the Supabase snapshot query+decode, and the SwiftData cache transaction, plus performance tests (`PerformanceMetricsTests`, `PerformanceUITests`) asserting CI-safe budgets. First CI run failed a 5s warm-relaunch threshold; diagnosed via raw job logs (not guessed) — every launch metric showed a consistent ~2-2.4x CI-vs-local slowdown across both matrix legs, confirming real CI hardware speed rather than a masked bug, so the threshold widened to 15s with the measured numbers documented inline. Greptile initial review found 2 real issues: a P1 (the UI test's "loaded" proxy — the search field — also appears during the loading skeleton, so the measurement could pass before real content rendered; fixed by waiting for an actual team row) and a P2 (the cache-transaction test swallowed errors via `try?` without asserting the round trip succeeded; fixed). CI-blocking thresholds are deliberately looser than the design spec's raw budget numbers to absorb CI-runner/XCUITest overhead — tighter spec-accurate numbers are only hard-asserted where the path is genuinely deterministic (in-memory cache transaction) or captured as an Instruments-visible signpost for human review; full reasoning in `.superpowers/sdd/2026-08-14-native-ios-app/task-9b-performance-report.md`. PR #371 squash-merged as c0b653f.

Task 9B: complete (PR #371, merge commit c0b653f, review clean after fix round)

Task 9C: shipped `ios/scripts/inspect-archive-secrets.sh` (builds Release/unsigned/iphonesimulator, scans Info.plist + compiled-binary `strings` output for service-role-shaped Supabase secrets) plus a new `archive-secret-check` CI job. Verification surfaced two real gaps in the brief's suggested approach before it ever hit CI: (1) the base64 substring for the local demo JWT's `"role":"service_role"` claim is byte-alignment-dependent — a single hardcoded substring (the brief's suggested one) silently misses the well-known Supabase local demo service-role JWT, which lands the claim at a different alignment; fixed by checking all three possible alignments, derivation documented inline and validated against 500 randomized trials. (2) `supabase-swift`'s own `APIKeyFormat.swift` hardcodes a bare `sb_secret_` prefix constant that compiles into every linking app's binary regardless of any real secret — a plain substring check would fail every build; fixed with a suffix-length-gated regex (`sb_secret_[A-Za-z0-9_-]{8,}`) so only key-shaped values trip it, with self-test fixtures covering both directions. Also fixed a `set -u`-unbound-variable bug in the derived-data cleanup trap (deferred single-quoted expansion of a function-local var) found only by running the real build-and-scan path, not just `--self-test`. Full reasoning in `.superpowers/sdd/2026-08-14-native-ios-app/task-9c-secret-inspection-report.md`.

Task 9D: shipped a `UI_TESTING_APPSTORE_SCREENSHOTS` launch mode (same shape as the existing `UI_TESTING_RESET_STATE`), a new `AppStoreScreenshotsUITests` suite capturing the five-screenshot sequence against the stable Bills fixture, and `docs/ios-appstore-screenshots.md`. Screenshot #4 (personal reorder editing) judgment call: `OverrideEditorViewModel` does no network I/O until `save()`, so a narrow `TeamDetailView.isAppStoreScreenshotMode` bypass opens the real reorder sheet in its authentic unsaved-drag-preview state without a real session — the sign-in gate is untouched for every other launch mode, and the test never taps Save. Excluded from CI via `project.yml`'s scheme `skippedTests` (no `ios-ci.yml` change needed) since it's a human-triggered release-prep tool, not a correctness gate. Greptile P1 (screenshot readiness checks satisfied by the wrong/prior screen for #4 and #5) fixed before merge. Flagged an unrelated pre-existing bug found while inspecting captured screenshots — `ScheduleView.swift` renders the season year as "2,026" instead of "2026" — as a separate background task rather than fixing it in-PR. PR #373 squash-merged as 25b2624.

Task 9D: complete (PR #373, merge commit 25b2624, review clean after fix round)

## T9 complete

All four T9 sub-tasks (9A CI workflow, 9B performance metrics, 9C archive-secret inspection, 9D screenshot automation) shipped as independently reviewed/merged PRs (#370-#373). Every implementation PR ran the full Staging `xcodebuild test` suite locally plus the new `ios-ci.yml` CI workflow (two-leg device matrix + archive-secret-check job) before merge, and addressed Greptile's initial review. T9A's own first real CI run surfaced two genuine defects the local-only verification habit across T2-T8 never caught (a device-runtime-compatibility gap in the CI script itself, and a real XCUITest race/below-the-fold rendering bug) — both root-caused by reproducing against the same device type CI uses rather than guessing from log timing, and are now documented for future sessions running native iOS CI work.
