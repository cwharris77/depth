...
// coverage: TeamBadgeOverride resolves the default primary-fill/secondary-ring
// badge for every team, and the three hand-curated overrides (Panthers keep their blue
// retired the Panthers row (it pinned an invented hex); Buccaneers swap the
...
private func team(id: String, primary: String, secondary: String, accent: String) -> Team {
    Team(
        id: id, city: "City", name: "Name", abbrev: id.uppercased(), conference: "AFC",
        division: "East",
...
private let panthers = team(
    id: "panthers", primary: "#0085CA", secondary: "#101820", accent: "#101820")
private let buccaneers = team(
    id: "buccaneers", primary: "#D50A0A", secondary: "#34302B", accent: "#FF7900")
private let broncos = team(
    id: "broncos", primary: "#FB4F14", secondary: "#002244", accent: "#002244")
...
// the Panthers row pinned the background to `uiAccent` (#36A7E0), a brightened
...
@Test func panthersNoLongerOverrideAndFallBackToRealPrimary() {
    ...
    #expect(
        TeamBadgeOverride.ringColorHex(for: panthers) == "#101820",
        "ring stays the default secondary")
}
...
@Test func broncosOverrideSwapsOrangePrimaryForNavyWithOrangeRing() {
    ...
    #expect(
        TeamBadgeOverride.ringColorHex(for: broncos) == "#FB4F14",
        "ring is the primary, not the default secondary")
}
