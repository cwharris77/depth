...
_Simple UI change (single screen, no complex flow): run `/ios-pr-screenshots` — it
captures the changed screen and fills the block below. Multi-screen or logic-heavy
change you're verifying in the simulator instead: skip the capture and replace the
block with one sentence justifying why. This section is required either way — not
optional silence._

<!-- screenshots-start -->

_Auto-generated by /ios-pr-screenshots, or a written justification for skipping capture._
