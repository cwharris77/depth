...
// unified into one component after that duplication carried a bug into both
...
// both screens).
//
// The "Back to current" action belongs next to the trigger that opened the sheet, in the
// same row, so it stays reachable without opening the sheet. `SeasonPickerTrigger` renders
// that affordance; the sheet does not need a second copy.
//
    ...
    var body: some View {
        ...
        layout {
            Button(action: action) {
                HStack(spacing: 4) {
                    // White stays legible against every team when the translucent material
                    // reduces contrast with the team's accent color.
            ...
            if isHistorical {
                ...
                // A text label keeps the action discoverable, and spacing it to the row's
                // far edge makes it a distinct control. Its capsule matches the trigger.
        ...
        DepthSheet(
            ...
            List(Array(items.enumerated()), id: \.element.id) { index, item in
                Button {
                    ...
                    // The full row, including the trailing spacer, must register taps.
