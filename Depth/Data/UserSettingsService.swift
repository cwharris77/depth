...
//
// Write contract mirrors web's PUT /api/settings exactly (web/app/api/settings/route.ts): a
...
/// In-memory mirror of web's `UserSettings` (`web/lib/utils/team/home-team.ts`). The default
/// value is the same EMPTY row web's API returns when none exists
/// (web/app/api/settings/route.ts's `EMPTY`): no favorite, no last-viewed, start-on-favorite
