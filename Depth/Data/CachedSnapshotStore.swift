...
actor CachedSnapshotStore {
    ...
    func saveTeamList(_ teams: [Team], cachedAt: Date) throws {
        ...
        for team in teams {
            modelContext.insert(
                CachedTeamListEntry(
                    team: team, schemaVersion: depthCacheSchemaVersion, cachedAt: cachedAt))
        }
        try modelContext.save()
    }
    ...
    func teamSnapshot(teamId: String) throws -> TeamSnapshot? {
        ...
        let state = DepthSignposts.signposter.beginInterval(
            DepthSignposts.teamSnapshotCacheTransaction, id: signpostID)
        defer {
            DepthSignposts.signposter.endInterval(
                DepthSignposts.teamSnapshotCacheTransaction, state)
        }
    ...
    private func validSnapshotRow(teamId: String) throws -> CachedTeamSnapshot? {
        var descriptor = FetchDescriptor<CachedTeamSnapshot>(
    ...
    func saveTeamSnapshot(_ snapshot: TeamSnapshot, teamId: String, cachedAt: Date) throws {
        ...
        if let existing = try modelContext.fetch(descriptor).first {
            ...
            modelContext.insert(
                CachedTeamSnapshot(
                    teamId: teamId, payload: payload, schemaVersion: depthCacheSchemaVersion,
                    cachedAt: cachedAt)
            )
        }
    ...
    private func enforceSnapshotCacheLimit(maxTeams: Int = 32) throws {
        let descriptor = FetchDescriptor<CachedTeamSnapshot>(sortBy: [
            SortDescriptor(\.cachedAt, order: .forward)
        ])
        let rows = try modelContext.fetch(descriptor)
    ...
    func teamStats(teamId: String) throws -> TeamStatsPage? {
        let signpostID = DepthSignposts.signposter.makeSignpostID()
        let state = DepthSignposts.signposter.beginInterval(
            DepthSignposts.teamStatsCacheTransaction, id: signpostID)
        defer {
            DepthSignposts.signposter.endInterval(DepthSignposts.teamStatsCacheTransaction, state)
        }
    ...
    private func validStatsRow(teamId: String) throws -> CachedTeamStats? {
        var descriptor = FetchDescriptor<CachedTeamStats>(
            predicate: #Predicate { $0.teamId == teamId })
        descriptor.fetchLimit = 1
        guard let row = try modelContext.fetch(descriptor).first else { return nil }
        guard row.schemaVersion == depthCacheSchemaVersion,
    ...
    func teamSchedule(teamId: String, season: Int?) throws -> TeamSchedule? {
        let signpostID = DepthSignposts.signposter.makeSignpostID()
        let state = DepthSignposts.signposter.beginInterval(
            DepthSignposts.teamScheduleCacheTransaction, id: signpostID)
        defer {
            DepthSignposts.signposter.endInterval(
                DepthSignposts.teamScheduleCacheTransaction, state)
        }
        guard
            let row = try validScheduleRow(
                cacheKey: scheduleCacheKey(teamId: teamId, season: season))
        else { return nil }
    ...
    private func validScheduleRow(cacheKey: String) throws -> CachedTeamSchedule? {
        var descriptor = FetchDescriptor<CachedTeamSchedule>(
    ...
    func saveTeamSchedule(_ schedule: TeamSchedule, teamId: String, season: Int?, cachedAt: Date)
        throws
    {
        let signpostID = DepthSignposts.signposter.makeSignpostID()
        let state = DepthSignposts.signposter.beginInterval(
        ...
        try upsertScheduleRow(
            cacheKey: scheduleCacheKey(teamId: teamId, season: season), payload: payload,
            cachedAt: cachedAt)
    }
    ...
    func saveDefaultSeasonSchedule(_ schedule: TeamSchedule, teamId: String, cachedAt: Date) throws
    {
        let payload = try JSONEncoder().encode(schedule)
        try upsertScheduleRow(
            cacheKey: scheduleCacheKey(teamId: teamId, season: nil), payload: payload,
            cachedAt: cachedAt)
        try upsertScheduleRow(
            cacheKey: scheduleCacheKey(teamId: teamId, season: schedule.season), payload: payload,
            cachedAt: cachedAt)
    }
    ...
    private func upsertScheduleRow(cacheKey: String, payload: Data, cachedAt: Date) throws {
        ...
        if let existing = try modelContext.fetch(descriptor).first {
            ...
            modelContext.insert(
                CachedTeamSchedule(
                    cacheKey: cacheKey, payload: payload, schemaVersion: depthCacheSchemaVersion,
                    cachedAt: cachedAt)
            )
        }
        try modelContext.save()
    ...
    func saveAppConfig(_ config: AppConfig, cachedAt: Date) throws {
        ...
        if let existing = try modelContext.fetch(descriptor).first {
            ...
            modelContext.insert(
                CachedAppConfig(
                    config: config, schemaVersion: depthCacheSchemaVersion, cachedAt: cachedAt))
        }
        try modelContext.save()
    }
    ...
    func saveUniformList(_ listings: [UniformListing], cachedAt: Date) throws {
        ...
        if let existing = try modelContext.fetch(descriptor).first {
            ...
            modelContext.insert(
                CachedUniformList(
                    cacheKey: key, payload: data, schemaVersion: depthCacheSchemaVersion,
                    cachedAt: cachedAt)
            )
        }
        try modelContext.save()
