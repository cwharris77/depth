...
- Data rhythm: unofficial ESPN APIs are ingested weekly (Wed 12:00 UTC), while nflverse
  ...
  (`web/lib/roster-source.db.ts`). A failed ingest can leave source data stale but never
  blocks a deploy.
...
- Native SwiftUI app at the repo root (same Supabase backend, its own design system) — the product. The frozen web app continues to serve the legal pages and share the backend.
...
- Live weekly pipeline produces real rosters for all 32 teams; a static seed fixture (`web/lib/teams/`) supports tests.
...
- WCAG AA contrast is a tested product requirement for every curated color pair (≥ 4.5:1 accent-on-accent pattern, enforced via data-looped tests like `web/lib/__tests__/uniforms.test.ts`).
