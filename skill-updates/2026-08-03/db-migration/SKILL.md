...
- **Every new table needs its own explicit grant line** — `grant select, insert,
  update, delete on <table> to anon, authenticated, service_role;` in the *same*
  migration as the table, right next to its RLS-enable line (precedent: any migration
  after `20260701171029_grant_default_table_privileges.sql`, which is a fixed table
  list and by construction cannot cover a table created later — don't treat it as
  "default privileges that might already apply"). This grant is required even for
  service-role-only tables with RLS-on/no-read-policy: GRANT and RLS are independent
  privilege layers, and `bypassrls` doesn't substitute for a missing GRANT — omitting
  it fails with `permission denied for table <name>` on first write.
