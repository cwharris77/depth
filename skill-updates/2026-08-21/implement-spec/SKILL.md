...
**A formal ticket with locked decisions and explicit Acceptance/Done-when criteria is
itself the scope confirmation** for CLAUDE.md's "restate the goal and wait before a
multi-file refactor" rule — proceed without a blocking pause even when the ticket
touches many files or a rename is ambiguous. Resolve any latitude the ticket leaves
(a naming/vocabulary mapping, an interpretation choice) as a judgment call and state
it explicitly in the PR/commit body so the reviewer can veto cheaply. Reserve the
blocking pause for genuinely underspecified asks — no acceptance criteria, or a
mapping choice that changes visible behavior.
