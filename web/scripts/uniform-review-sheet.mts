...
function readBefore(root: string, base: string, rowId: string): Buffer | undefined {
  try {
    return execFileSync(
      'git',
      ['-C', root, 'show', `${base}:web/public/uniforms/${rowId}-full.webp`],
      { encoding: 'buffer' }
    );
  } catch {
    return undefined;
...
async function loadReferences(
  ...
  for (const file of files) {
    const path = join(teamDir, file);
    const raster = await sharp(path, { animated: false })
  ...
  for (const [rowId, crop] of Object.entries(crops)) {
    const path = join(teamDir, crop.file);
...
function loadReviewNotes(refsDir: string | undefined, teamId: string): ReviewNotes {
  ...
  const path = join(refsDir, teamId, 'review.json');
...
async function main() {
  ...
  for (const row of teamRows) {
    ...
    const beforeBuffer = readBefore(root, base, row.id);
  ...
  const outPath = out ?? join(os.tmpdir(), `uniform-review-${team}.html`);
  const resolvedOut = isAbsolute(outPath) ? outPath : resolve(WEB_ROOT, outPath);
  if (resolvedOut.startsWith(root + '/') || resolvedOut === root) {
    throw new Error(`refusing to write the review sheet inside the repo: ${resolvedOut}`);
  }
