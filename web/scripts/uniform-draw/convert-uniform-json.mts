...
// Pure so it's unit-testable without the outline toolchain: given a validated definition and
// its already-outlined jerseys, build the plain-object parts the generated module exports.
// `socks` is only emitted when authored, so a module generated before socks existed stays
// byte-identical when regenerated from JSON that still doesn't author them.
export function buildParts(definition: JsonDefinition, jerseys: Record<string, JsonJersey>) {
  return {
    ...
    ...(definition.socks && { socks: definition.socks }),
...
function main() {
  ...
  if (!input || !output) {
    throw new Error('usage: convert-uniform-json.mts <input.json> <output.ts>');
  }
  ...
  const parts = buildParts(definition, jerseys);
