...
# The Ignored Build Step runs with cwd set to the Vercel Root Directory (web/), so `.`
# scopes the diff to the deployable tree: a change that touches only repo-root docs
# (README, CLAUDE.md, the vault) produces no web/ diff, exits 0, and the build is skipped.
