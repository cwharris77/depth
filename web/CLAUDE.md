...
**The product is the native iOS app** (repo root) — an interactive NFL depth chart viewer:
...
Postgres · XcodeGen. Read [`../CLAUDE.md`](../CLAUDE.md) for its operating manual.
...
ESPN unofficial APIs
  ...
  → iOS DepthRepository        (../Depth/Data/DepthRepository.swift — the iOS seam)
...
new feature work happens in iOS, whose parallel invariants live in [`../CLAUDE.md`](../CLAUDE.md) §2
...
4. **A kit stores three colors; the app decides where each one goes.** `primary/secondary/
   ...
   `../Depth/Domain/TeamSurfaces.swift` with 105-kit fixture parity. Call sites ask for a
...
- **Conventional Commits** for commits and PR titles. Scopes in use: `ios`, `uniforms`,
  ...
  scopes `ios` (see [`../CLAUDE.md`](../CLAUDE.md)). **`design:` is for a change to how something looks
...
cached-read crashes, reaching around `DepthRepository`) are in [`../CLAUDE.md`](../CLAUDE.md) §4.
...
**iOS app (additionally)** — see [`../CLAUDE.md`](../CLAUDE.md) for the full iOS
operating manual (architecture, conventions, parity mechanisms). Quality-bar summary:
- [ ] Targeted test runs only, never the full suite: `xcodebuild -project Depth.xcodeproj
