...
import { toTeamStatsRows, type TeamSeasonStatsInsert } from '../lib/nflverse/team-stats';
...
// nflverse release tag for team-level stats, asset stats_team_reg_<season>.csv.
// Mirror of stats_player convention; same season range (1999–present). Unlike player
// stats, team_season_stats has no player FK — so --seasons backfill works for the
// full range without a player-identity decision (docs/superpowers/specs/2026-08-01-
// historic-nflverse-coverage-design.md).
...
async function main() {
  ...
  // Team-season stats (team_season_stats): nflverse stats_team tag, cumulative per
  // season, FK to teams (no player-identity issue), so --seasons backfill works for
  // the full available range.
  const teamStatsSeasons = seedOut
    ? seasons
    : gamesSeasons ?? seasons;
  let teamStatsRowsWritten = 0;
  let teamStatsSkipped = 0;
  const allTeamStatsRows: TeamSeasonStatsInsert[] = [];

  for (const season of teamStatsSeasons) {
    try {
      const url = assetUrl(TEAM_STATS_TAG, `${TEAM_STATS_PREFIX}${season}.csv`);
      const statsCsv = await getText(url);
      const { rows, skipped: seasonSkipped } = toTeamStatsRows(parseCsv(statsCsv));
      teamStatsSkipped += seasonSkipped;
      if (supabase && rows.length) {
        const { error } = await supabase
      ...
      allTeamStatsRows.push(...rows);
      teamStatsRowsWritten += rows.length;
      console.log(`team-stats ${season}: wrote ${rows.length} rows, skipped ${seasonSkipped}`);
    } catch (e) {
      failures.push({ season: `team-stats-${season}`, message: (e as Error).message });
    }
  }
  ...
  if (seedOut) {
    const parts = [
      ...
      '-- nflverse player-stats/team-stats/schedules/games/formations snapshot for local `supabase db',
      '-- reset`. Requires supabase/seed.sql to already be loaded (players FK). Do not',
      '-- hand-edit; regenerate.',
      '',
      buildPlayerStatsSeedSql(allStatsRows),
      buildTeamSeasonStatsSeedSql(allTeamStatsRows),
    ...
    console.log(
      `\nWrote seed: ${allStatsRows.length} player-stat rows, ${allTeamStatsRows.length} team-stat rows, ` +
  ...
  const { error: runError } = await supabase.from('ingestion_runs').insert({
    ...
    errors: {
      ...
      team_season_stats_rows: teamStatsRowsWritten,
  ...
  console.log(
    `\nWrote ${rowsWritten} player-stat rows, ${teamStatsRowsWritten} team-stat rows across ${seasons.length} season(s), ` +
