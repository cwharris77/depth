...
This is a contour trace of the supplied standalone club mark, not a trace of the
low-resolution helmet thumbnail.  The source is ``/Users/cwharris/Downloads/2026_Baltimore.svg``
(non-free/trademarked artwork supplied for this pass; it is never committed here).

Topology, measured from a 2400px true-aspect raster of that SVG before tracing:
The visible source has separate gold, purple, white, black, and eye components.
The eye is gold in the supplied vector but is intentionally emitted as red to
match the helmet reference.  The large white page background is border-connected
and discarded; black is deliberately not emitted because the helmet shell
supplies those negative-space gaps.  The selection trap is treating the gold eye
ring as part of the gold silhouette; the smallest interior gold component is
isolated and recolored red.
...
that is ``Box(180.0, 125.0, 380.0, 190.0)``.  The mark is allowed to overhang
...
REF = Path('/Users/cwharris/Downloads/2026_Baltimore.svg')
MODULE = Path(__file__).resolve().parents[2] / 'lib' / 'uniforms' / 'teams' / 'ravens.ts'
RENDER_WIDTH = 2400
EPS = 2.4
MIN_REGION = 100

BOX = Box(180.0, 125.0, 380.0, 190.0)
...
def smallest_interior_mask(labels, name):
    """Return the smallest non-border component for the supplied ink colour."""
    h, w = len(labels), len(labels[0])
    raw = [[1 if labels[y][x] == name else 0 for x in range(w)] for y in range(h)]
    regions = [
        region
        for region in components(raw, w, h, MIN_REGION)
        if not any(x in (0, w - 1) or y in (0, h - 1) for x, y in region)
    ]
    eye = set(min(regions, key=len))
    return [[1 if (x, y) in eye else 0 for x in range(w)] for y in range(h)]
...
def build():
    ...
    eye = smallest_interior_mask(labels, 'gold')
    gold = ink_mask(labels, 'gold')
    gold = [
        [value and not eye[y][x] for x, value in enumerate(row)]
        for y, row in enumerate(gold)
    ]
    return {
        'RAVENS_DECAL_GOLD_PATH': trace(crop(gold), art_width, art_height, BOX, eps=EPS, minsize=MIN_REGION, space='image'),
        ...
        'RAVENS_DECAL_EYE_PATH': trace(crop(eye), art_width, art_height, BOX, eps=EPS, minsize=MIN_REGION, space='image'),
    }
