...
import { STORAGE_KEY_DEPTH_KIT_COLORS } from '@/lib/utils/storage-keys';
...
// Written by lib/hooks/depth-chart/use-kit.ts whenever the roster page's active kit (Home/Away/throwback/…)
...
export function writeKitColors(teamId: string, colors: TeamColors) {
  try {
    ...
    // lib/hooks/depth-chart/use-kit.ts's writeStoredKit for the same reasoning.
  }
}
...
// Home — the same "global state" expectation as lib/hooks/use-last-accent.ts's nav-wide
...
export function useKitColors(team: TeamMeta): TeamColors {
  ...
  // lib/hooks/depth-chart/use-kit.ts's identical roster.uniforms reset for the reasoning.
  ...
  // default colors first — same reasoning as lib/hooks/depth-chart/use-kit.ts's kit restoration.
  useLayoutEffect(() => {
