...
import { getKitId, setKitId as persistKitId } from '@/lib/utils/depth-chart/kit-selection';
import { writeKitColors } from '@/lib/hooks/use-kit-colors';
...
// changes.
//
// Kit choice is persisted per team to localStorage (lib/utils/depth-chart/kit-selection.ts) so it survives
...
export function useKit(roster: TeamRoster) {
  ...
  // Mirror the resolved colors into lib/hooks/use-kit-colors.ts's store so schedule/stats —
