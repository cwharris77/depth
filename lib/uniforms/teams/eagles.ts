// Philadelphia's construction geometry — the wing decal paths, cuff and collar paths, and the
// black construction literal only. The composable parts definition that consumes them lives in
// ./eagles.parts.ts; the former flat EAGLES_UNIFORMS was deleted in the migration that proved
...
// sleeve hem. The kelly-green throwback drops the cuff entirely and keeps only the collar. No
// helmet stripe, no pant stripe on any kit.

// Black is a literal on the two current kits. Their palettes are green over silver with accent ===
// secondary on the home, so neither can resolve the collar yoke or the sleeve cuff, both of which
// the reference draws black.
...
// (Wikimedia `File:Philadelphia Eagles logo.svg`, fair use; trademarked). Licence audit: the
// vault’s Decisions.md, 2026-09-03.
//
// The wing: two plain-union fills in paint order, never evenodd holes — the outline (black ∩ white,
// so the union carries the whole wing) under the white body. The feather lines inside come for
// free as the gaps between the white components.
...
// The sleeve cuff, extended outward to x=30 and past the hem so the jersey clip trims it.
export const EAGLES_CUFF_LEFT = 'M30,545 H140 V578 H30 Z';
export const EAGLES_CUFF_RIGHT = 'M558,545 H448 V578 H558 Z';

// The collar yoke, deeper than the generic chevron.
export const EAGLES_COLLAR_PATH = 'M211,389 L294,481 L377,389';
export const EAGLES_COLLAR_WIDTH = 20;
