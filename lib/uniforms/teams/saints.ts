// New Orleans' construction geometry — the fleur-de-lis path and the collar-width constant
// only. The composable parts definition that consumes them lives in ./saints.parts.ts; the former
// flat SAINTS_UNIFORMS was deleted in the migration that proved parts render byte-identically (see
...
// mapped onto the raw helmet space at ~6.25x. It is a solid black shape and traces cleanly — what
// is genuinely too fine is the white line the reference draws INSIDE it, which is barely two px
// and has no separable predicate. So the shape is authored as one solid fill with that line
// dropped, which at swatch size is indistinguishable and at 4x reads as a slightly bolder fleur.
