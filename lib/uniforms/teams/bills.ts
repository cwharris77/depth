import type { TeamUniformDefinition, UniformLayer } from './types';
...
// (Wikimedia `File:Buffalo Bills logo.svg`, fair use; trademarked). Licence audit: the vault’s
...
// BILLS_DEFAULT_LAYERS paints the silhouette first and stripe second; the stripe's unfilled notch
// intentionally lets the white shell show through instead of adding another paint layer.
...
// Back-edge piping also stays in raw helmet space. Stroke geometry preserves its thin-line role;
// the wider navy outer stroke must paint before the narrower red inset in BILLS_DEFAULT_LAYERS.
...
// The GUD construction reference shows red/white/navy as explicit bands: white disappears against
// the away jersey and navy against the home jersey, but neither is transparent. These paths use the
...
// Painting every band explicitly keeps the same construction on both white and blue jerseys.
...
// Array order is paint order within each surface: silhouette before diagonal stripe, wider edge
// piping before its inset, and widest collar stroke before the narrower strokes layered over it.
const BILLS_DEFAULT_LAYERS: UniformLayer[] = [
  {
...
const AWAY_RED_LAYERS: UniformLayer[] = [
  {
...
// Rivalries replaces the color helmet marks and intentionally omits the standard sleeve, collar,
// and pants banding: its frost-textured shoulder is represented by the absence of colored marks.
const RIVALRIES_REMOVED_LAYERS = [
  'bills-helmet-buffalo',
  'bills-helmet-stripe',
  'bills-helmet-edge-outer',
  'bills-helmet-edge-inner',
  'bills-sleeve-red-left',
  'bills-sleeve-red-right',
  'bills-sleeve-white-left',
  'bills-sleeve-white-right',
  'bills-sleeve-navy-left',
  'bills-sleeve-navy-right',
  'bills-collar-white',
  'bills-collar-red',
  'bills-collar-navy',
...
export const BILLS_UNIFORMS: TeamUniformDefinition = {
  teamId: 'bills',
  defaults: {
    // Every referenced Bills kit keeps a white shell, independent of its runtime body palette.
    helmetColor: '#FFFFFF',
    removeLayerIds: [
    ...
    layers: BILLS_DEFAULT_LAYERS,
  },
  kits: {
    home: {},
    // Away keeps the shared white shell/bands but replaces generic stripe layers with red and uses
    // Buffalo's fixed navy number plus red outline instead of contrast-derived number colors.
    away: {
      layers: AWAY_RED_LAYERS,
      number: { fill: BILLS_NAVY, outline: BILLS_RED },
    },
    // Rivalries (2025-) is the tone-on-tone "ice" alternate: silver/navy helmet treatment, no red
    // or standard shoulder/collar/pants bands, and a silver number with a navy outline. The number
    // silver #A9ADB1 is the curated approximation used for the official product-shot reference.
    'rivalries-2025': {
      removeLayerIds: RIVALRIES_REMOVED_LAYERS,
      layers: [
        {
          id: 'bills-helmet-buffalo',
          surface: 'helmet',
      ...
      number: { fill: '#A9ADB1', outline: BILLS_NAVY },
    },
  },
};
